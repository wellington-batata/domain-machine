...
from utils.domains import split_into_batches, domains_to_jsonl , build_domains, numbers_filter, size_filter, special_chars_filter
...
def process_batch_upload(domains_list: list[str], bucket_name: str, file_key: str):
  chunk_size = int(os.getenv("CHUNK_SIZE", 150))
  dir_batches = f"{os.getenv("DIR_BATCHES", "batches")}/{file_key}"
  char_limit = int(os.getenv("CHAR_LIMIT", 16))
  ...
  print(f"Total de domínios após filtro de extensão: {len(domains_clean)}")
  domains_clean = numbers_filter(domains_clean)[1]
  print(f"Total de domínios após filtro de números: {len(domains_clean)}")
  domains_clean = size_filter(domains_clean, char_limit=char_limit)[1]
  print(f"Total de domínios após filtro de tamanho: {len(domains_clean)}")
  domains_clean = special_chars_filter(domains_clean, preference_ext)[1]
  print(f"Total de domínios após filtro de caracteres especiais: {len(domains_clean)}")
